...
func (i *reportATLUseCase) Get(start int, limit int, jenis string, g *gin.Context) ([]models.ReportBMDATL, int64, int64, int64, interface{}, error) {
	...
	return i.GetData(start, limit, tgl, pidopd, pidopd_cabang, pidupt, tahun, bulan, draw, jenis, jenisperiode, tglakhir)
...
func (i *reportATLUseCase) GetData(start int, limit int, tgl string, pidopd string, pidopd_cabang string, pidupt string, tahun string, bulan string, draw string, jenis string, jenisperiode string, tglakhir string) ([]models.ReportBMDATL, int64, int64, int64, interface{}, error) {
	...
	// pre query
	params := i.db.Raw(`select ? tahun_sekarang, ? tahun_sebelum, ?::text tanggal, ?::text pidopd, ?::text pidopd_cabang, ?::text pidupt, ?::text tglakhir`, tahun_sk, tahun_sb, tgl, pidopd, pidopd_cabang, pidupt, tglakhir)
	...
	if jenisperiode == "1" {
		sqlQuery = sqlQuery.Where(" TO_CHAR(i.tgl_dibukukan, 'yyyy-mm') <= pr.tanggal ")
	} else {
		sqlQuery = sqlQuery.Where("i.tgl_dibukukan >= CAST(pr.tanggal AS date)").Where("i.tgl_dibukukan <= ?", tglakhir)
	}
	sqlQuery = sqlQuery.Order("i.id")
	...
	strWhere := fmt.Sprintf(`
		...
		i.deleted_at IS NULL AND i.draft IS NULL`, pidopd, pidopd, pidopd_cabang, pidopd_cabang, pidupt, pidupt)

	if jenisperiode == "1" {
		strWhere = fmt.Sprintf(`%s AND TO_CHAR(i.tgl_dibukukan, 'yyyy-mm') <= '%s' `, strWhere, tgl)
	} else {
		strWhere = fmt.Sprintf(`%s AND i.tgl_dibukukan <= '%s' AND i.tgl_dibukukan >= '%s'`, strWhere, tgl, tglakhir)
	}
...
func (i *reportATLUseCase) Export(start int, limit int, f_periode string, f_penggunafilter string, penggunafilter string, f_kuasapengguna_filter string, kuasapengguna_filter string, f_subkuasa_filter string, subkuasa_filter string, f_tahun string, f_bulan string, f_jenis string, action string, firstload string, draw string, jenisperiode string) ([]models.ReportBMDATL, int64, int64, int64, interface{}, error) {
	...
	return i.GetData(start, limit, tgl, pidopd, pidopd_cabang, pidupt, f_tahun, f_bulan, draw, "export", jenisperiode, tglakhir)
